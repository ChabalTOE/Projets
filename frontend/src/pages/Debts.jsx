...
function Debts(){
  ...
  const loadDebts=async()=>{
    const res=await axios.get(`${API_BASE}/api/dettes/`,{headers})
    setDebts(res.data)
  }
  ...
  useEffect(()=>{ if(token){ loadDebts(); loadCategories() } },[token])

  const saveDebt=async()=>{
    if(!montant || !categorie || !quantite){ setMessage('Montant, catégorie et quantité requis'); return }
    await axios.post(`${API_BASE}/api/dettes/`, {montant:parseFloat(montant),categorie,quantite:parseInt(quantite),date},{headers})
    setMontant(''); setCategorie(''); setQuantite(''); setMessage('Dette ajoutée avec succès')
    loadDebts()
  }
  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        <div>
          <h2 style={{margin:0,color:'#1f2a44'}}>Gestion des dettes</h2>
          <p style={{margin:'8px 0 0',color:'#606f8a'}}>Enregistrez les crédits clients selon les catégories de produits.</p>
        </div>
      </div>
      ...
      <div style={{background:'white',borderRadius:24,padding:24,boxShadow:'0 18px 50px rgba(15,23,42,0.08)',marginBottom:24}}>
        ...
        <button onClick={saveDebt} style={{padding:'14px 20px',borderRadius:14,background:'#3751ff',color:'white',border:'none',cursor:'pointer'}}>Enregistrer dette</button>
      </div>

      <div style={{background:'white',borderRadius:24,padding:24,boxShadow:'0 18px 50px rgba(15,23,42,0.08)'}}>
        <h3 style={{margin:'0 0 18px',color:'#1f2a44'}}>Liste des dettes</h3>
        <div style={{display:'grid',gap:14}}>
          {debts.map(item=>(
            <div key={item.id} style={{padding:'16px',borderRadius:18,background:'#f3f6ff'}}>
          ...
          {debts.length===0 && <p style={{color:'#6f7a94'}}>Aucune dette enregistrée.</p>}
        </div>
      </div>
    </div>
  )
